...
import { Config } from "./Library/Backend/Config";
...
import { Letture } from "./Library/Backend/Letture";
import { Walk, FSWalk } from "./Library/Backend/Walk";
...
ipcMain.handle("request-save-branch-custom", async (event: Electron.IpcMainEvent, id: number, branchObjectCustom: IBranchCustom) => {
    letture.exec(`UPDATE "library" SET
        "custom" = '${JSON.stringify(branchObjectCustom)}'
    WHERE
        "id" = ${id}
    ;`);
    const resultBranchObjectCustom: IBranchCustom = <IBranchCustom>JSON.parse(<string>letture.prepare(`SELECT
        "custom"
    FROM
        "library"
    WHERE
