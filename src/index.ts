...
import OS from "os";
import { Trait } from "./Library/Common/Trait";
...
const createWindow = (): void => {
  ...
  (<any>process).mainWindow = new BrowserWindow({
    width: 1800,
    height: 1000,
    //fullscreen: true,
    webPreferences: {
  ...
  (<any>process).mainWindow.setIcon(PATH.resolve(__dirname,"pianissimo.png"));
  // and load the index.html of the app.
  (<any>process).mainWindow.loadURL(MAIN_WINDOW_WEBPACK_ENTRY);
  // Open the DevTools.
  (<any>process).mainWindow.webContents.openDevTools();
...
const trait = new Trait("test", {a:1, b:2, c:3});
